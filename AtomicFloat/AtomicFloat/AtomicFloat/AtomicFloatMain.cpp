...
{
	//! アプリケーションを実行する
	int AtomicFloatMain::Main()
	{
		try
		{
			// 計算を実行
			AtomicFloatMain::Compute();
		}
	...
	//! 計算する
	void AtomicFloatMain::Compute()
	{
		// 実行開始
		cout << "= 浮動小数点atomic操作テスト =" << endl;

		// OpenCL Cソースファイル名
		const string filepath = "AtomicFloat.cl";

		// エントリポイント名
		const string entryPoint = "SumWithAtomicFloat";

		// ベクトルの要素数
		const cl_uint elementCount = 1234;
